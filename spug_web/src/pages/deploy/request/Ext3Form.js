...
const SOURCE_TYPES = ['branch', 'tag', 'repository', 'docker_image'];
...
export default observer(function () {
  ...
  function handleSubmit() {
    ...
    if (extra1 === undefined || extra1 === null || extra1 === '') {
      return message.error('请选择要发布的分支、标签、版本或镜像')
    }
  ...
  function _initial(versions, repositories, env, images) {
    if (env.env_prod) {
      return _setDefault('tag', [], versions, repositories, images)
    }
    ...
    if (branches && tags) {
      ...
      _setDefault('branch', [], versions, repositories, images)
    }
  }
