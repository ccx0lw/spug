...
from apps.docker_images.models import DockerImages
from apps.deploy.models import DeployRequest
from apps.docker_images.utils import dispatch
from apps.app.models import Deploy
from threading import Thread
import json


class DockerImagesView(View):
    @auth('deploy.docker_images.view|deploy.request.add|deploy.request.edit')
    ...
    def post(self, request):
        ...
        if error is None:
            ...
            form.spug_version = DockerImages.make_spug_version(deploy.id)
            rep = DockerImages.objects.create(
    ...
    @auth('deploy.docker_images.add|deploy.docker_images.build')
    def patch(self, request):
        ...
        if error is None:
            rep = DockerImages.objects.filter(pk=form.id).first()
            if not rep:
    ...
    def delete(self, request):
        ...
        if error is None:
            docker_images = DockerImages.objects.filter(pk=form.id).first()
            if not docker_images:
                return json_response(error='未找到指定构建记录')
            if docker_images.deployrequest_set.exists():
                return json_response(error='已关联发布申请无法删除')
            docker_images.delete()
        return json_response(error=error)


@auth('deploy.docker_images.view')
...
def get_detail(request, r_id):
    docker_images = DockerImages.objects.filter(pk=r_id).first()
    if not docker_images:
        return json_response(error='未找到指定构建记录')
    rds, counter = get_redis_connection(), 0
    if docker_images.remarks == 'SPUG AUTO MAKE':
        req = docker_images.deployrequest_set.last()
        key = f'{settings.REQUEST_KEY}:{req.id}'
    else:
        key = f'{settings.BUILD_KEY}:{docker_images.spug_version}'
    data = rds.lrange(key, counter, counter + 9)
    response = AttrDict(data='', step=0, s_status='process', status=docker_images.status)
    while data:
        for item in data:
            counter += 1
    ...
    if docker_images.status in ('0', '1'):
